...
setup_ainish_coder_dir() {
  ...
  mkdir -p "${AINISH_CODER_DIR}/cursor" 2>/dev/null
  mkdir -p "${AINISH_CODER_DIR}/vscode" 2>/dev/null
  mkdir -p "${AINISH_CODER_DIR}/aider" 2>/dev/null
  
  # Copy all configuration files to ~/.ainish-coder
  cp -r "${REPO_DIR}/ainish-cursor/"* "${AINISH_CODER_DIR}/cursor/" 2>/dev/null
  cp -r "${REPO_DIR}/ainish-copilot/"* "${AINISH_CODER_DIR}/vscode/" 2>/dev/null
  cp -r "${REPO_DIR}/ainish-aider/"* "${AINISH_CODER_DIR}/aider/" 2>/dev/null
  
  # Verify and create cursor ignore files if needed
  if [ ! -f "${AINISH_CODER_DIR}/cursor/.cursorignore" ]; then
    touch "${AINISH_CODER_DIR}/cursor/.cursorignore" 2>/dev/null
    echo -e "${GREEN}✓ Created empty .cursorignore in ${AINISH_CODER_DIR}/cursor/${RESET}"
  fi
  
  if [ ! -f "${AINISH_CODER_DIR}/cursor/.cursorindexingignore" ]; then
    touch "${AINISH_CODER_DIR}/cursor/.cursorindexingignore" 2>/dev/null
    echo -e "${GREEN}✓ Created empty .cursorindexingignore in ${AINISH_CODER_DIR}/cursor/${RESET}"
  fi
  
  # Copy the license-citation.mdc to the root ~/.ainish-coder directory
  cp "${REPO_DIR}/license-citation.mdc" "${AINISH_CODER_DIR}/" 2>/dev/null
  
  # Copy the setup script itself
  cp "${REPO_DIR}/ainish-setup.sh" "${AINISH_CODER_DIR}/" 2>/dev/null
