...
### The Expert-Guided Workflow

1.  **STRATEGY**: Ralph queries `osa-expert` via MCP `query_expert_strategy` to understand the codebase impact and past lessons.
2.  **PLAN**: Ralph updates `TODO.md` with the "Strategic Plan" provided by the expert.
3.  **DELEGATE**: Ralph spawns **Gemini** or **Qwen** branches using the specific "Directives" generated by the expert.
    - `gemini -y -p "{expert_directive_for_gemini}"`
    - `qwen -y "{expert_directive_for_qwen}"`
4.  **SYNC**: Sub-agents update `OSAVARS.md` with their results.
5.  **MERGE**: Ralph reviews results, merges code, and stores any new "Lessons Learned" back into `osa-expert`.

### Agent Specialization Guide (Refined)
...
| **Gemini** | Branch | Large Context (2M+), Multimodal (Images/PDFs) | **Complex Implementation**, Documentation, Architecture Diagrams |
| **Qwen** | Branch | High Speed, Low Latency, Precise Summarization | **Code Review**, Quick Fixes, Summarizing Logs/Tests |
| **Opencode** | Branch | Standard Coding Patterns | Pure Boilerplate, Simple Logic |

---

## Expert Loop (SOTA 2026)

```
User → Trunk (Ralph) → Expert Core (Rag-Anything/Memori/DSPy) → Execution
```

**Use when:** Deep codebase understanding or high-fidelity architectural reasoning is required.

**Pattern:**
```bash
# 1. Bootstrap (once)
python -m osa_expert bootstrap

# 2. Query Expert for Strategy
python -m osa_expert query "How should we refactor the auth layer?" --role Architect

# 3. Execute with Sub-agents (Sequential/Parallel)
# ... standard execution ...

# 4. Commit Lessons Learned
python -m osa_expert commit "Refactored auth layer using hybrid JWT/PQC" --outcome Success --lesson "Ensure X25519 is used for key exchange"
```
