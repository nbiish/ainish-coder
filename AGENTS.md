...
The calling AI agent operates as an **Augmented Fleet Orchestrator Master**, possessing full authority and conviction to command, prompt-engineer, and supervise the **Subagent Master Fleet** — terminal engines (`trae-cli`, `mini`) and harness-native subagent modalities (`subagent`, `subagent_fork`, `workflow`, `ralph`). All subagents auto-route through single-config proxy `http://127.0.0.1:11434/v1` (`local-router/fallback-models`).
...
3. **Tier 3: The Subagent Masters:** The surgical hands. Every modality is invoked as a direct tool call in dedicated worktrees under loopback proxy `11434`:
...
The four canonical task templates — `TPL_TRAE_AST_V2` (AST Refactoring Master, trae-cli), `TPL_MINI_TDD_REPRO_V1` (TDD Reproduction Engineer, mini), `TPL_SECURITY_AUDIT_V1` (Adversarial Security Auditor), and `TPL_TRAE_SYSTEMS_V1` (Systems Architecture Master) — live in **`.agents/skills/orchestrate-subagent-masters/SKILL.md` §3** and are the ONLY authoritative copies. AGENTS.md deliberately does not inline them (drift prevention).
...
1. **PRE-FLIGHT (shell one-liner):** before the first dispatch of a session, verify engine binaries resolvable on `PATH`, loopback proxy `11434` health (`curl -sf http://127.0.0.1:11434/api/version`), backend `11435` (warn-only), and dedicated-worktree isolation. `NO-GO` = fix the environment first; zero fleet steps on a broken env. Loopback probes only — never probe non-loopback hosts.
2. **DISPATCH (direct invocations):** every dispatch is one fixed, non-interactive command vector executed directly by the orchestrator under `timeout` — `trae-cli run -f <task.md> --console-type simple --patch-path <patch> --max-steps 30` (prompt via task file) · `mini --task "<task>" --yolo --exit-immediately` (never `--config`) · native modalities (`subagent` / `subagent_fork` / `workflow` / `ralph`) through their harness tool-call forms with the same scope allowlist and gates embedded in the prompt. Parallel dispatches only for independent scopes; otherwise run synchronously.
...
5. **EXIT TAXONOMY (orchestrator-normalized handoff):** `0` OK -> proceed · `20` step-exhausted (zero edits) -> hand to sibling · `30` probe-loop (mini >=3 identical probes) -> hand failure signature to trae-cli · `40` engine/gates failed -> fix scope, re-dispatch · `50` scope violation -> revert, tighten allowlist, re-dispatch · `60` preflight failed -> fix environment · `124` timeout -> treat as `20`. Post-dispatch scrubbing (`rm -f` task/trajectory files) is a mandatory invariant — a dispatch whose artifacts cannot be purged never proceeds to merge.
