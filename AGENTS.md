...
### Engine Capability & Circuit Breaker Matrix (DeepSeek Harness)

| Engine | Core Strengths & Expert Persona | Master Invocations & Directives (Executed as Tool Calls) | Circuit Breaker & Fallback |
|---|---|---|---|
| **`dsh --profile headless`** (DeepSeek Harness) | One-shot scoped task execution — structural edits, TDD reproduction, audits, systems work. Answers one task, prints the final message, exits; persists a fresh session per run.<br>*Personas:* **AST Refactoring Master**, **TDD Reproduction Engineer**, **Adversarial Security Auditor**, **Systems Architecture Master** | `cd ../<slug> && timeout 1800 dsh --profile headless "$(cat /tmp/task.md)"`<br>Templates: `TPL_AST_REFACTOR_V3`, `TPL_TDD_REPRO_V3`, `TPL_SECURITY_AUDIT_V3`, `TPL_SYSTEMS_ARCH_V3`. Write prompt to `<task.md>` first. | Step-exhausted (zero edits) $\rightarrow$ re-dispatch a sibling scoped task with discovered targets; boot/config/auth failure $\rightarrow$ `60`, fix environment — never hand off. |
| **`dsh --profile acp`** (DeepSeek Harness) | Persistent automation surface: multi-turn agent sessions over ACP stdio for editors and orchestrator clients.<br>*Persona:* set by the driving client | Boot deliberately as a managed background process with bounded lifetime; never inside a one-shot dispatch. | Session misbehaves $\rightarrow$ disconnect, collect the session log as an artifact, re-dispatch one-shot instead. |

### Dynamic Handoff Chaining (Autonomous Tool Pipeline)
- **Refactor $\rightarrow$ Harden:** Calling agent (embodying AST Master) dispatches a `dsh headless` tool call to perform structural refactoring $\rightarrow$ calling agent (embodying TDD Engineer) dispatches a fresh `dsh headless` tool call to synthesize reproduction tests and harden edge-case coverage.
- **Probe $\rightarrow$ Fix:** Calling agent (embodying TDD Engineer) dispatches `dsh headless` to isolate the bug with a minimal failing test $\rightarrow$ calling agent (embodying AST Master) dispatches a scoped follow-up to apply the production patch to exact files.
- **Zero Interaction Mandate:** One-shot dispatches never prompt — `dsh headless` answers and exits; interactive profiles (`web`/`tui`/`acp`) are never booted inside a dispatch.

### Canonical Templates (Single Source of Truth)

The four canonical task templates — `TPL_AST_REFACTOR_V3` (AST Refactoring Master), `TPL_TDD_REPRO_V3` (TDD Reproduction Engineer), `TPL_SECURITY_AUDIT_V3` (Adversarial Security Auditor), and `TPL_SYSTEMS_ARCH_V3` (Systems Architecture Master) — live in **`.agents/skills/orchestrate-subagent-masters/SKILL.md` §3** and are the ONLY authoritative copies. AGENTS.md deliberately does not inline them (drift prevention).

Template invariants (non-negotiable): scoped `SCOPE & TARGET FILES` allowlist; one-shot engines only inside dispatches (`dsh --profile headless` answers and exits; never boot `web`/`tui`/`acp` in a dispatch); task prompts via task file fed with `"$(cat <file>)"`; dispatch only inside dedicated sibling worktrees (`../<slug>`, never `main` — the invoking directory is the workspace root); engine inference follows the booted profile's provider config, loopback-only when routed through local-router.

### Dispatch Protocol v3 — Terminal-Native Preflight, Dispatch, Receipts, Exit Taxonomy (Binding)

No wrapper scripts: the orchestrator runs plain, fixed commands and normalizes outcomes itself.

1. **PRE-FLIGHT (shell one-liner):** before the first dispatch of a session, verify the `dsh` launcher on `PATH` (`command -v dsh && dsh --version`) and run the inference round-trip pong (`timeout 120 dsh --profile headless "Reply with the single word: pong. Do not run any commands or modify any files." | grep -q pong`), plus dedicated-worktree isolation. `NO-GO` = fix the environment first; zero fleet steps on a broken env. **Liveness is not health** — the pong proves the profile's provider completes inference; when a profile routes through local-router, additionally run the loopback `/v1/chat/completions` round-trip (loopback probes only — never probe non-loopback hosts).
2. **DISPATCH (direct invocations):** every dispatch is one fixed, non-interactive command vector executed directly by the orchestrator under `timeout`, from inside the dispatch worktree — `dsh --profile headless "$(cat /tmp/task.md)"` (prompt via task file; headless answers and exits) · persistent multi-turn work runs through a deliberately managed `dsh --profile acp` process, never inside a one-shot dispatch · native modalities (`subagent` / `subagent_fork` / `workflow` / `ralph`) through their harness tool-call forms with the same scope allowlist and gates embedded in the prompt. Parallel dispatches only for independent scopes; otherwise run synchronously.
...
4. **RECEIPTS (COMMS ledger = the receipt):** one `SUBAGENT-DISPATCH` ledger entry per dispatch (schema in the skill §6): modality/engine, persona, branch, scope allowlist, changed files, scope verdict, gate results, native exit code, normalized code — attributed `parent: <orchestrator>`. **No ledger receipt = the dispatch never happened.**
5. **EXIT TAXONOMY (orchestrator-normalized handoff):** `0` OK -> proceed · `20` step-exhausted (zero edits) -> hand to sibling modality · `30` probe-loop (>=3 identical probes) -> hand failure signature to a fresh scoped dispatch · `40` engine/gates failed -> fix scope, re-dispatch · `50` scope violation -> revert, tighten allowlist, re-dispatch · `60` environment (preflight failure, dsh boot/config/foreign-option failure, auth/upstream-error signature) -> fix environment, never hand off · `124` timeout -> treat as `20`. Post-dispatch scrubbing (`rm -f` task files; session logs under `$DSH_HOME` treated as artifacts) is a mandatory invariant — a dispatch whose artifacts cannot be purged never proceeds to merge.
