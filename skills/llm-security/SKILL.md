...
| # | Vulnerability | Code Defense |
|---|---------------|-------------|
| LLM01 | Prompt Injection | Input sanitization; XML separation; output monitoring |
| LLM02 | Sensitive Info Disclosure | PII redaction (presidio); output filtering; data classification |
| LLM03 | Supply Chain | SLSA compliance; SBOMs; vendor risk assessment; hash pinning |
| LLM04 | Data & Model Poisoning | Data provenance tracking; embedding anomaly detection |
| LLM05 | Improper Output Handling | Strict schema validation (Zod/Pydantic); never exec LLM output |
| LLM06 | Excessive Agency | Explicit approval workflows; read-only defaults; HITL |
| LLM07 | System Prompt Leakage | Canary tokens; no credentials in prompts; instruction isolation |
| LLM08 | Vector & Embedding Weakness | RAG input validation; signed context; provenance verification |
| LLM09 | Misinformation | Multi-model consensus; verification steps; citations required |
| LLM10 | Unbounded Consumption | Rate limiting; resource quotas; token budgets per task |

### OWASP Agentic Top 10 (ASI 2026)

| # | Vulnerability | Mitigation | Real CVE |
|---|---------------|-----------|----------|
| ASI01 | Agent Goal Hijack | Immutable system instructions; separate control/data planes | CVE-2025-32711 |
| ASI02 | Tool Misuse | Schema validation (Zod/Pydantic); per-tool rate limits | Amazon Q incidents |
| ASI03 | Identity & Privilege Abuse | Independent Permission Broker; short-lived tokens; JIT access | Credential exploitation |
| ASI04 | Supply Chain | SLSA compliance; SBOM + AIBOM; dependency pinning by hash | ClawHavoc: 341 malicious skills |
| ASI05 | Unexpected Code Execution | Sandbox all execution (WASM/Firecracker); validate FFI | CVE-2025-54135 |
| ASI06 | Memory & Context Poisoning | Verify RAG provenance; cryptographic context signatures | Moltbook: 506 injections |
| ASI07 | Insecure Inter-Agent Comms | mTLS with PQC certs (ML-DSA-65); signed messages | Spoofing/interception |
| ASI08 | Cascading Failures | Circuit breakers; token budget limits; graceful degradation | Multi-agent faults |
| ASI09 | Human-Agent Trust Exploit | Explicit approval workflows; confidence scores; citations | Replit DB deletion |
| ASI10 | Rogue Agents | Alignment monitoring; behavior bounds; immutable audit trail | CVE-2025-53773 |

### OWASP Agentic Skills Top 10 (AST10 2026)

| # | Risk | Fix |
|---|------|-----|
| AST01 | Malicious Skills | Merkle root signing; code review before install |
| AST02 | Supply Chain | Registry transparency logs; provenance verification |
| AST03 | Over-Privileged Skills | Schema validation; least-privilege scoping |
| AST04 | Insecure Metadata | Static analysis of skill manifests |
| AST05 | Prompt Injection via Skills | Prompt sanitization; instruction isolation |
| AST06 | Weak Isolation | Containerized skill execution; process isolation |
| AST07 | Update Drift | Immutable version pinning; hash verification |
| AST08 | Poor Scanning | Multi-tool scanning pipeline (SAST + DAST) |
| AST09 | No Governance | Skill inventories; approval workflows |
| AST10 | Cross-Platform Reuse | Universal skill format (YAML); cross-platform testing |
