...
handle_error() {
    ...
    echo -e "${RED}ERROR: $1${NC}" >&2 # Print error to stderr
    exit 1
}
...
copy_file() {
    ...
    if ! $PERFORM_COPIES; then
        ...
        echo "Skipping copy of $src to $dst (copy operations disabled)"
        return
    fi
    ...
    echo -e "${color}Successfully copied $src to $dst${NC}"
}
...
if confirm_action "Perform all file copy operations?"; then
    ...
    echo "File copy operations enabled"
else
    # log "File copy operations disabled"
    echo "File copy operations disabled"
fi
...
if [ ! -f "PRD.mdc" ] && $PERFORM_COPIES; then
    ...
    echo "PRD.mdc not found in root directory"
    ...
    if [ -f ".cursor/rules/PRD.mdc" ]; then
        ...
        echo "Successfully copied PRD.mdc from .cursor/rules/ to root directory"
        ...
        echo "Successfully copied PRD.mdc from ainish-cursor/.cursor/rules/ to root directory"
        ...
        echo "Successfully copied PRD.mdc from ainish-aider/ to root directory"
    else
        # log "Could not find PRD.mdc in any location to copy to root"
        echo "Could not find PRD.mdc in any location to copy to root"
    fi
fi

# Copy critical.mdc from root to destinations
# log "Processing critical.mdc distribution..."
echo "Processing critical.mdc distribution..."
if [ -f "critical.mdc" ]; then
    ...
    echo "critical.mdc not found in root directory"
fi

# Copy PRD.mdc from root to destinations
# log "Processing PRD.mdc distribution..."
echo "Processing PRD.mdc distribution..."
if [ -f "PRD.mdc" ]; then
...
if $PERFORM_COPIES; then
    ...
    echo "No copy operations performed (disabled by user)"
fi
